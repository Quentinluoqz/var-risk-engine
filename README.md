...
![CI](https://github.com/Quentinluoqz/var-risk-engine/actions/workflows/ci.yml/badge.svg)
...
The offline demo produces a full HTML report plus model diagnostics. The chart below is generated by the rolling out-of-sample VaR backtest.

![Rolling VaR backtest example](docs/assets/backtesting_example.png)
...
├── data/
│   ├── sample_prices.csv
│   └── cache/                # Local download cache, ignored by Git
├── configs/
│   └── demo.yaml
...
The exact GARCH-MC, EVT, and copula values vary with the latest cached data and model fits. The project reports these values directly in the pipeline output and stores supporting figures under each run's `figures/` directory.
